// HDRP's Hidden/HDRP/VolumetricCloudsCombine (com.unity.render-pipelines.high-definition
// 17, Unity 6.6), copied with one change: the fog stops reaching into the clouds at
// the Fog's Max Fog Distance.
//
// HDRP fogs the sky as if it were Max Fog Distance away (64 in the Game scene), but
// it fogs the clouds over their real distance, which is kilometres. So the height
// fog around the arena, which never ends sideways, buried every cloud near the
// horizon, and lowering the Fog Attenuation Distance erased the clouds while the
// sky behind them stayed clear. Here the clouds get exactly the fog the sky gets.
...
Shader "Hidden/Survival Chaos/Volumetric Clouds Combine"
{
    ...
    SubShader
    {
        ...
        // Survival Chaos: the depth the fog is evaluated at for a cloud. No
        // further than Max Fog Distance along the ray, which is where HDRP
        // already puts the sky (AtmosphericScattering.hlsl, isSky). The cosine
        // is the one EvaluateAtmosphericScattering divides by to turn this back
        // into a distance along the ray.
        float CloudFogDepth(float linearDepth, float3 V)
        {
            return min(linearDepth, _MaxFogDistance * dot(-V, GetViewForwardDir()));
        }
        ...
        Pass
        {
                ...
                )
            {
                ...
                posInput.linearDepth = CloudFogDepth(linearDepth, V);
        ...
        Pass
        {
            ...
            )
            {
                ...
                // Apply fog. On a copy, because the refraction sort below wants
                // the cloud's real depth.
                PositionInputs fogInput = posInput;
                fogInput.linearDepth = CloudFogDepth(linearDepth, V);
                float3 volColor, volOpacity;
                EvaluateAtmosphericScattering(fogInput, V, volColor, volOpacity);
